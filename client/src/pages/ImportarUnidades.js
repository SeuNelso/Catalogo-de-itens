...
const ImportarUnidades = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/importar-unidades', {
        method: 'POST',
        headers: {
  ...
  const downloadTemplate = async () => {
    try {
      ...
      const response = await fetch('/api/download-template-unidades', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      if (response.ok) {
        ...
        link.href = url;
        link.download = 'template_unidades.xlsx';
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Importar Unidades de Armazenamento
          </h1>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          {isUploading && (
            <div className="mb-6">
              ...
              <div className="mt-2 text-xs text-gray-500">
                ...
                {uploadProgress >= 30 && uploadProgress < 80 && 'Validando e processando unidades...'}
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            >
              {isUploading ? 'Importando...' : 'Importar Unidades'}
            </button>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="space-y-4">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-left py-2 px-3 bg-gray-100">UNIDADE_ARMAZENAMENTO</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-b">
                      <td className="py-2 px-3">3000003</td>
                      <td className="py-2 px-3">UN</td>
                    </tr>
                    <tr className="border-b">
            ...
            <div>
              <h3 className="font-medium text-gray-900 mb-2">Unidades Válidas:</h3>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 text-sm">
                {[
                  'UN', 'KG', 'M', 'L', 'PÇ', 'ROL', 'CAIXA', 'PACOTE',
                  'METRO', 'LITRO', 'QUILO', 'PECA', 'UNIDADE'
                ].map(unidade => (
                  <span key={unidade} className="bg-blue-50 text-blue-700 px-2 py-1 rounded">
                    {unidade}
                  </span>
                ))}
              </div>
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-1 text-sm text-gray-600">
                ...
                <li>Unidades inválidas serão ignoradas</li>
                <li>Uma unidade por item (não aceita múltiplas unidades)</li>
                <li>As unidades são convertidas para maiúsculas automaticamente</li>
                <li>Unidades vazias não alteram o valor existente</li>
              </ul>
            </div>
          </div>
        </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 max-h-60 overflow-y-auto">
                  {importResult.detalhes.slice(0, 10).map((detalhe, index) => (
                    <div key={index} className="text-sm text-gray-700 mb-2 p-2 bg-white rounded border">
                      ...
                      {detalhe.unidadeInvalida && (
                        <div className="text-xs text-red-600 mt-1">
                          Unidade inválida: {detalhe.unidadeInvalida}
                        </div>
                      )}
                    </div>
                  ))}
