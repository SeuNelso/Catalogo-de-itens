...
function quantidadeNecessariaStockPreparacao({
  ...
  if (isTipoControloSerial(t) && Array.isArray(serialsNormalizados) && serialsNormalizados.length > 0) {
...
function ordemTipoMovimentoClog(tipo) {
  ...
  if (t === 'devolucao de carrinha') return 2;
  return 9;
}
...
router.get('/movimentos-clog/consulta', ...requisicaoAuth, denyOnlyOperador, async (req, res) => {
  try {
    ...
    const normalizarTransferenciaCentralCentral = (row) => {
      ...
      if (tipoAtual === 'devolucao de carrinha' || tipoAtual === 'apeados') return row;
...
router.patch('/:id/atender-item', ...requisicaoAuth, denyBackofficeOperations, async (req, res) => {
  ...
  try {
    ...
    if (!Number.isInteger(quantidadePreparadaRaw)) {
